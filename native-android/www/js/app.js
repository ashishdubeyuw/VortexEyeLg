...
class VortexEyeApp {
    ...
    async init() {
        try {
            ...
            this.cacheElements();
            this.logger = window.vxLog || null;

            await this.handleSignedOtaUpdates();
    ...
    async handleSignedOtaUpdates() {
        ...
        try {
            const manifestUrl = `${window.VortexEyeOTASecurity.MANIFEST_URL}?t=${Date.now()}`;
            const resp = await fetch(manifestUrl, { cache: 'no-store' });
            if (!resp.ok) {
                throw new Error(`Manifest fetch failed with status ${resp.status}`);
            }

            const manifest = await resp.json();
            const manifestCheck = await window.VortexEyeOTASecurity.verifyManifest(manifest);
            if (!manifestCheck.ok) {
                throw new Error(manifestCheck.error);
            }
            ...
            this.showLoading(`Verifying v${remoteVer} package...`);
            const bundleCheck = await window.VortexEyeOTASecurity.verifyBundle(manifest.url, manifest.sha256);
            if (!bundleCheck.ok) {
                throw new Error(bundleCheck.error);
            }
    ...
    initializePrivacyControls() {
        ...
        if (this.elements.diagnosticsToggle) {
            this.elements.diagnosticsToggle.checked = this.logger.diagnosticsEnabled;
            this.elements.diagnosticsToggle.disabled = !this.logger.consentGranted;
        }

        const consentState = localStorage.getItem('vortex_privacy_consent');
        if (!consentState && this.elements.privacyOverlay) {
            this.elements.privacyOverlay.classList.remove('hidden');
        }
    }

    handlePrivacyChoice(granted) {
        if (!this.logger) return;
        this.logger.setConsent(granted);
        if (this.elements.diagnosticsToggle) {
            this.elements.diagnosticsToggle.disabled = !granted;
            this.elements.diagnosticsToggle.checked = granted && this.logger.diagnosticsEnabled;
        }
        if (this.elements.privacyOverlay) {
    ...
    cacheElements() {
        this.elements = {
            ...
            privacyDeclineBtn: document.getElementById('privacyDeclineBtn')
        };
    }
    ...
    setupEventListeners() {
        ...
        if (this.elements.settingsBtn) {
            this.elements.settingsBtn.addEventListener('click', () => {
                ...
                if (this.elements.diagnosticsToggle && this.logger) {
                    this.elements.diagnosticsToggle.checked = this.logger.diagnosticsEnabled;
                    this.elements.diagnosticsToggle.disabled = !this.logger.consentGranted;
                }
            });
        ...
        if (this.elements.diagnosticsToggle) {
            this.elements.diagnosticsToggle.addEventListener('change', (e) => {
                if (!this.logger) return;
                if (!this.logger.consentGranted && e.target.checked) {
                    e.target.checked = false;
                    this.elements.privacyOverlay?.classList.remove('hidden');
                    return;
                }
                this.logger.setDiagnosticsEnabled(e.target.checked);
            });
        }
        ...
        if (this.elements.aboutBtn) {
            this.elements.aboutBtn.addEventListener('click', () => {
                ...
                if (valVersion) {
                    valVersion.textContent = `Version ${localStorage.getItem('ve_version') || '1.2.0'}`;
                }
                if (valNotes) {
                    valNotes.textContent = `Patch Details: ${localStorage.getItem('ve_notes') || 'Security hardening release with local-only diagnostics and native Android packaging.'}`;
                }
        ...
        if (this.elements.privacyAcceptBtn) {
            this.elements.privacyAcceptBtn.addEventListener('click', () => this.handlePrivacyChoice(true));
        }

        if (this.elements.privacyDeclineBtn) {
            this.elements.privacyDeclineBtn.addEventListener('click', () => this.handlePrivacyChoice(false));
        }
    ...
    showSuggestions(results, targetField) {
        ...
        results.forEach((result) => {
            ...
            item.dataset.name = shortName;
            item.dataset.target = targetField;
    ...
    hideSuggestions() {
        ...
        this.elements.suggestionsDropdown.replaceChildren();
    }
